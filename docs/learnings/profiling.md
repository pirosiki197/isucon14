...
nginx の `access_log off` で測り直した。結果は **125.1s → 123.6s（1.2%）** で、
スコアも変動幅の内側だった。`wa=0.4%` の通り**シーケンシャルな追記は安い**。

つまり**計装を入れたまま最後までチューニングして構わない**。
最終計測前に切る手間も要らなかった。切る前提で設計を歪める必要はない。
